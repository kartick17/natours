...
exports.signup = catchAsync(async (req, res, next) => {
  if (req.body.role && req.body.role === 'admin') {
    return next(new AppError('User can not create admin profile', 400));
  }
  req.body.photo =
    'https://natours-image.s3.ap-south-1.amazonaws.com/default.jpg';

  const newUser = await User.create(req.body);

  const url = `${req.protocol}://${req.get('host')}/me`;
  const emailFrom = `The Natours Team <community@natours.in>`;
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError('Token is invalid or expried', 400));
